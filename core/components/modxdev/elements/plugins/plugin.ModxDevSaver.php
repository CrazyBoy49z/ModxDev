...
switch ($modx->event->name)
{
	case 'OnTempFormRender':
		$config = array(
			...
			'source' => $modx->getOption('modxdev.static_media_source').'/template',
			'fileExt' => 'modxdev.static_template_file_extension',
		);
		break;
	case 'OnChunkFormRender':
		$config = array(
			...
			'source' => $modx->getOption('modxdev.static_media_source').'/chunk',
			'fileExt' => 'modxdev.static_chunk_file_extension',
		);
		break;
	case 'OnSnipFormRender':
		$config = array(
			...
			'source' => $modx->getOption('modxdev.static_media_source').'/snippet',
			'fileExt' => 'modxdev.static_snippet_file_extension',
		);
		break;
	case 'OnPluginFormRender':
		$config = array(
			...
			'source' => $modx->getOption('modxdev.static_media_source').'/plugin',
			'fileExt' => 'modxdev.static_plugin_file_extension',
		);
		break;
	case 'OnTVFormRender':
		$config = array(
			...
			'source' => $modx->getOption('modxdev.static_media_source').'/tv',
			'fileExt' => 'modxdev.static_tv_file_extension',
		);
		break;
	default: $config = null;
		break;
...
if ($config)
{
	...
	$modx->regClientStartupScript('<script src="' . $modx->getOption('assets_url') . 'components/modxdev/inc/js/staticsaver.js"></script>');
